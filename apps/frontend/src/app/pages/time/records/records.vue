    ...
    <form class="filter-form" @submit.prevent="handleSubmit">
      ...
      >
        <div class="flex flex-col">
          ...
          >
            <option selected :value="'checkIns'">
              {{ $t('timeRecords.checkIns') }}
            </option>
            <option :value="'checkOuts'">
              {{ $t('timeRecords.checkOuts') }}
            </option>
          </select>
        </div>
        <div class="flex flex-col">
          <label for="startDate">{{ $t('timeRecords.startDate') }}</label>
        ...
        <div class="flex flex-col">
          <label for="endDate">{{ $t('timeRecords.endDate') }}</label>
          <FormattableDateField
            :datePattern="dateFormat"
            :placeholder="datePlaceholder"
            class="!border-dark-4 !bg-white !text-dark-2 !font-normal text-base w-full md:w-full"
        ...
        <div class="flex flex-col">
          <label for="employeeName">{{ $t('timeRecords.employeeName') }}</label>
          <input
            type="text"
            id="employeeName"
            name="employeeName"
