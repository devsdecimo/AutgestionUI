<template>
    ...
    <div class="submenu-container">
      <a class="submenu-container__item btn--small" href="/time">
        {{ $t('time.timeRegistration') }}
      </a>
      <a class="submenu-container__item btn--small btn--outline" href="/time/records">
        {{ $t('time.timeRecordQuery') }}
      </a>
    </div>

    <div class="main-container">
      <div class="p-10 md:p-20 bg-white drop-shadow-lg rounded-3xl gap-10 w-full m-auto flex flex-col">
        <Clock :timezone="selectedTimezone?.value || 'America/Costa_Rica'" />

        <div class="w-full xl:w-2/5 2xl:w-1/5 m-auto flex flex-col gap-10">
          <Dropdown :options="timezones" :icon="['fas', 'location-dot']" :value="selectedTimezone" @onOptionSelected="handleTimezoneChange" class="whitespace-nowrap"/>
  ...
  const handleTimezoneChange = (option: { label: string; value: string } | null) => {
